# Thane War II: Shadow of the Horde HD

The HD-remastered sequel to Thane War — the same retro real-time strategy game (canvas + WebAudio, no external assets). Play the
...
- `prebuilt/web/` — the live game, deployed to `/apps/thane-war-2/`
- `prebuilt/downloads/thane-war-2.html` — self-contained single-file download
  (all JS/CSS inlined; runs offline straight from `file://`)

## Rebuilding after source changes

```sh
npm run pack:thane-war-2   # from the repo root
```
...
The source must be present at `apps/thane-war-2/src/`. The root dev server
(`npm run dev`) serves the game with hot reload at `/apps/thane-war-2/`.
